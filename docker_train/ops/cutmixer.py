...
class CutMixer(object):
    ...
    #      x1 = (ch - h.floor_divide(2)).clamp(0, H).tolist()
    #      y1 = (cw - w.floor_divide(2)).clamp(0, W).tolist()
    #      x2 = (ch + h.floor_divide(2)).clamp(0, H).tolist()
    #      y2 = (cw + w.floor_divide(2)).clamp(0, W).tolist()
    #      for ii, (xx1, xx2, yy1, yy2) in enumerate(zip(x1, x2, y1, y2)):
    #          ims[ii, :, xx1:xx2, yy1:yy2] = ims[indices[ii], :, xx1:xx2, yy1:yy2]
    #      lam = 1. - ((x2 - x1) * (y2 - y1)).float() / (H * W).float()
    #
    #      lam = lam.view(-1, 1)
